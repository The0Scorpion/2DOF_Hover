...
fn='MQTTTest'+year+"-"+month+"-"+date+"--"+hours+":"+minutes;
console.log(fn);
...
function onMessageArrived (m: Message) {
  ...
  var pkt=JSON.parse(m.payloadString);
  /*console.log(pkt);
  console.log(pkt["ID"]);
  console.log(pkt["Time"]);*/
  document.getElementById("form_field_settings_dialog").innerText += pkt["ID"]+","+m.payloadBytes.byteLength +","+pkt["Time"]+","+Date.now()+","+(Date.now()-pkt["Time"])+"\r\n";
